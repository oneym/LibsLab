...
public class DownloadPost {
    ...
    //单例
    private static DownloadPost instance = null;
    //线程池   20151215155142
    private ExecutorService pool = null;

    //构造方法
    private DownloadPost() {
    }

    /**
     * @return 单例
     */
    public static DownloadPost getInstance() {
        if (null == instance)
            instance = new DownloadPost();
        return instance;
    }
     ...
     * @param listener 监听接口
     */
    private void start(final int distinct, final String url, final Map<String, String> params, final String savepath, final String filename, final OnDownloadingListener listener) {
        try {
...
//                pool = Executors.newCachedThreadPool();
            ...
            pool.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    file.deleteOnExit();
                    //开始下载
                    _download(url, params, savepath, filename, listener);
                }
            });
     ...
     * @param listener 监听接口
     */
    public void start(Context context, String url, final Map<String, String> params, String filename, OnDownloadingListener listener) {
        Map map = getData(context, url, filename, listener);
        start(0, map.get("url").toString(), params, map.get("savepath").toString(), map.get("filename").toString(), listener);
    }
     ...
     * @param listener 监听接口
     */
    public void start(String url, final Map<String, String> params, String filename, String SDHome, OnDownloadingListener listener) {
        Map map = getData(url, filename, SDHome, listener);
        start(0, map.get("url").toString(), params, map.get("savepath").toString(), map.get("filename").toString(), listener);
    }

    //核心下载方法
    private void _download(final String url, final Map<String, String> params, final String savepath, final String filename, final OnDownloadingListener listener) {
        //数据输入流
        ...
        try {
            ...
            if (null == params || params.size() <= 0) {
                if (null != listener)
                    listener.onMessage(OnDownloadingListener.TYPE_ERROR, "参数不可为空");
                Log.out("参数不可为空");
            }
            ...
            post.setEntity(new UrlEncodedFormEntityHC4(pairs, "UTF-8"));
            ...
            if (HttpURLConnection.HTTP_OK == response.getStatusLine().getStatusCode()) {
                ...
                if (lenAll == len || response.getEntity().isChunked()) {
